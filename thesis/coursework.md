...
cache для эффективных sweeps. Дополнительно проект подключает Moment-DETR
raw-video path, адаптированный из official external repository. Moment-DETR
сначала проверялся как 50-query feasibility probe, а затем был оценен на полном
Charades-STA test split. Это coursework-scale reproducible experiment, а не
official QVHighlights benchmark или прямое SOTA comparison.
...
этой курсовой Moment-DETR используется через raw-video path с compatible
checkpoint. Он сначала применялся как isolated 50-query probe, а затем как
full-test Charades-STA comparison. Это не является полноценным
воспроизведением official QVHighlights benchmark.
...
### 4.6 Moment-DETR raw-video integration
...
pipeline. Первоначально она проверялась на 50-query feasibility probe; после
этого тот же path был запущен на полном Charades-STA test split.
...
Historical 50-query CLIP vs Moment-DETR feasibility probe сохранен здесь:
...
Final full-test CLIP vs Moment-DETR comparison сохранен здесь:
...
### 6.3 CLIP vs Moment-DETR на полном Charades-STA test split

Финальное CLIP vs Moment-DETR comparison использует полный Charades-STA test
split: 3,720 queries / 1,334 videos. Оба метода оцениваются на одном и том же
manifest, подготовленном из:
...
CLIP использует выбранную по 1,000-query sweep configuration
`clip_w16_s8_mean`. Moment-DETR использует `moment_detr_raw_video` с compatible
raw-video checkpoint из external repository. Source file:
`results/clip_vs_moment_detr_full_test/comparison_summary.csv`.
...
На полном test split CLIP имеет более высокий `R@1 IoU 0.3` и `mIoU`.
Moment-DETR имеет более высокие strict metrics при `R@1 IoU 0.5` и
`R@1 IoU 0.7`. CLIP быстрее в этом CPU experiment, но сохраняет больший cache
embeddings. Эти результаты следует читать как reproducible coursework
experiments на общей локальной evaluation setup, а не как прямое SOTA
comparison.
...
Moment-DETR full-test result показывает, что интеграция specialized temporal
localization model технически возможна на полном Charades-STA test split, но
остается чувствительной к checkpoint и feature format. Raw-video-compatible
checkpoint работает с локальными Charades videos. Released QVHighlights feature
checkpoint несовместим с raw-video path из-за feature dimension mismatch.
Поэтому rigorous Moment-DETR benchmark потребовал бы более аккуратного
environment и feature setup, чем coursework-scale comparison в этой курсовой.

## 8. Ограничения

CLIP window/stride sweep и smoothing ablation используют fixed 1,000-query
subset, а не полный Charades-STA test split. Эта subset нужна для
контролируемого выбора configuration. Финальное CLIP vs Moment-DETR comparison
выполнено на полном Charades-STA test split.
...
Moment-DETR comparison использует raw-video-compatible checkpoint из external
repository и не fine-tuned на Charades-STA. 50-query result остается
historical feasibility probe; финальный reported comparison выполнен на полном
Charades-STA test split. Full official QVHighlights benchmarking, feature
...
Финальное full-test comparison показывает разные сильные стороны моделей:
CLIP `clip_w16_s8_mean` выше по coarse `R@1 IoU 0.3 = 0.5944` и
`mIoU = 0.3348`, а Moment-DETR выше по strict `R@1 IoU 0.5 = 0.2570` и
`R@1 IoU 0.7 = 0.0995`. Это подтверждает, что specialized temporal model
можно подключить к локальным данным, но full official benchmark и fine-tuning
